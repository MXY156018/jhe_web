		...
		<div class="gva-search-box">
			<el-form ref="searchForm" :inline="true" :model="searchInfo">
				...
				<el-form-item label="反馈内容">
					<el-input v-model="searchInfo.name" placeholder="反馈内容,支持模糊查找" />
				</el-form-item>
			...
			<div class="gva-table-box">
				<div class="gva-btn-list">
					<el-popover v-model:visible="deleteVisible" placement="top" width="160">
						...
						<template #reference>
							<el-button icon="el-icon-delete" size="mini" :disabled="!tableData.length"
				...
				<el-table :data="tableData" @sort-change="" @selection-change="">
					...
					<el-table-column align="left" label="用户id" min-width="50" prop="uid" />
					<el-table-column align="left" label="反馈内容" min-width="50" prop="message" />
					
					<el-table-column align="center" label="反馈时间" width="180" type="date">
						<template #default="scope">{{ formatDate(scope.row.create_time) }}</template>
					</el-table-column>
					<el-table-column align="center" label="解决时间" width="180" type="date">
						<template #default="scope">{{ formatDate(scope.row.resolve_time) }}</template>
					</el-table-column>
					<el-table-column align="center" label="删除时间" width="180" type="date">
						<template #default="scope">{{ formatDate(scope.row.delete_time) }}</template>
					</el-table-column>
					<el-table-column align="left" label="状态" min-width="50">
						<template #default="scope">{{asyncStatus(scope.row.status)}}</template>
					</el-table-column>
					<el-table-column align="left" fixed="right" label="操作" width="100">
						<template #default="scope">
							<el-button icon="el-icon-edit" size="small" type="text" @click="editFeedBack(scope.row)">查看
							</el-button>
							<el-button v-if="scope.row.status!='2'" icon="el-icon-delete" size="small" type="text" @click="deleteFeedBack(scope.row)">删除
							</el-button>
						</template>
					</el-table-column>
				</el-table>
			...
			<el-dialog v-model="dialogFormVisible" title="查看反馈">
				<el-form  label-width="80px">
					<el-form-item label="uid" >
						<el-input v-model="feedDetail.uid" autocomplete="off" autosize disabled/>
					</el-form-item>
					<el-form-item label="反馈内容">
						<el-input type="textarea" autosize disabled v-model="feedDetail.message"   autocomplete="off" />
					</el-form-item>
				...
				<template #footer>
					<div class="dialog-footer">
						...
						<el-button size="small" v-if="feedDetail.status='0'" type="primary" @click="enterDialog">解 决</el-button>
					</div>
				</template>
			</el-dialog>
		</div>
