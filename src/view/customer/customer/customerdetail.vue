		...
		<div class="detail">
			<el-form  label-width="auto" >
				<el-form-item label="ID:" >
					<template #default="scope">{{ detail.id }}</template>
				</el-form-item>
				<el-form-item label="地址:">
					<template #default="scope">{{ detail.address }}</template>
				</el-form-item>
				<el-form-item label="註冊時間:">
					<template #default="scope">{{ formatDate(detail.create_time) }}</template>
					<!-- <el-input  disabled  :value="handlerTime" autocomplete="off" /> -->
				</el-form-item>
				
				<el-form-item label="錢包餘額:">
					<template #default="scope">{{ detail.balance }}</template>
				</el-form-item>
				<el-form-item label="類型:">
					<template #default="scope">{{ setType(detail.type) }}</template>
				</el-form-item>
				<el-form-item label="下級節點總充值:">
					<template #default="scope" style="justify-content: space-between;">{{ detail.sum_subordinate_recharge }} JHE</template>
				</el-form-item>
			</el-form>
		</div>
			...
			<el-tabs v-model="activeName" tab-position="left" @tab-click="handleClick">
				<el-tab-pane label="下級" name="first">
					<el-table :data="first_data">
						...
						<el-table-column align="center" label="類型" min-width="50">
							<template #default="scope">{{ setType(scope.row.type)}}</template>
						</el-table-column>
						<el-table-column align="center" label="註冊時間" width="180" type="date">
							<template #default="scope">{{ formatDate(scope.row.create_time) }}</template>
						</el-table-column>
					</el-table>
					<div class="pagination" :hide-on-single-page='true'>
						<el-pagination :current-page="page1" :page-size="pageSize1" :page-sizes="[10, 30, 50, 100]" :total="total1"
							layout="total, sizes, prev, pager, next, jumper" @current-change="handleCurrentChange1"
							@size-change="handleSizeChange1" />
					</div>
				</el-tab-pane>
				<el-tab-pane label="遊戲記錄" name="second">
					<el-table :data="second_data">
						<el-table-column align="center" label="時間" width="180" type="date">
							<template #default="scope">{{ formatDate(scope.row.end_time) }}</template>
						</el-table-column>
						<el-table-column align="center" label="遊戲模式" min-width="50" >
							<template #default="scope">{{ setGameType(scope.row.type)}}</template>
						</el-table-column>
						<el-table-column align="center" label="遊戲前餘額" min-width="50" >
							<template #default="scope">{{scope.row.pre_balance}} JHE</template>
						</el-table-column>
						<el-table-column align="center" label="得分" min-width="50" >
							<template #default="scope">{{scope.row.win}} JHE</template>
						</el-table-column>
						<el-table-column align="center" label="遊戲後餘額" min-width="50" >
							<template #default="scope">{{scope.row.now_balance}} JHE</template>
						</el-table-column>
					</el-table>
					<div class="pagination" :hide-on-single-page='true'>
						<el-pagination :current-page="page2" :page-size="pageSize2" :page-sizes="[10, 30, 50, 100]" :total="total2"
							layout="total, sizes, prev, pager, next, jumper" @current-change="handleCurrentChange2"
				...
				<el-tab-pane label="操作記錄" name="third">
					<el-table :data="third_data">
						...
						<el-table-column align="center" label="類型" min-width="50" >
							<template #default="scope">{{ setOperateType(scope.row.type)}}</template>
						</el-table-column>
					...
					<div class="pagination" :hide-on-single-page='true'>
						<el-pagination :current-page="page3" :page-size="pageSize3" :page-sizes="[10, 30, 50, 100]" :total="total3"
							layout="total, sizes, prev, pager, next, jumper" @current-change="handleCurrentChange3"
